...
float L_X = 1.4f, L_Y = 22.0f, L_Z = 0.8f;
...
#define cnt_models 9
vector<GLuint> VBO(cnt_models, 0), VAO(cnt_models,0), VBT(cnt_models,0), 
	VBN(cnt_models,0), IBO(cnt_models,0), counts(cnt_models,0);
...
void loadModels()
{
	...
	v = { 0,0,0 };
	int i = 0;
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\grass.obj", 
		v, 1.5);

	++i;
	v = { 1, 4.1, 4 };
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\Bowl.obj", 
		v, 1/12.f);

	++i;
	v = { 5.3, 0, 2 };
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\apple textured obj.obj", 
		v, 1 / 80.f);

	++i;
	v = { 6, 0.5, 1.5 };
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\pear_export.obj",
		v, 1 / 6.f);

	++i;
	v = { 1, 0, 2 };
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\Pizza and Juice.obj",
		v, 1 / 5.f);
	...
	v = { 0, 0, 0 };
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\cola.obj",
		v, 1 / 110.6f);
	...
	v = { 4.5, 0, 4.5 };
	counts[i] = initVBO(VBO[i], VBN[i], VBT[i], IBO[i],
		"D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\objs\\jolka.obj",
		v, 1 / 25.0f);
}
...
void render()
{
	...
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\tabletex.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);

	++i;
	m.material_shininess = 70.0f;
	m.material_specular = { 0.7, 0.7, 0.7, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\metal-texture-green.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);

	++i;
	m.material_shininess = 70.0f;
	m.material_specular = { 0.7, 0.7, 0.7, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\appleD.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);

	++i;
	m.material_shininess = 70.0f;
	m.material_specular = { 0.7, 0.7, 0.7, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\pear_diffuse.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);

	++i;
	m.material_shininess = 10.0f;
	m.material_specular = { 0.1, 0.1, 0.1, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\pizza-home.png");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);
	...
	++i;
	m.material_shininess = 120.0f;
	m.material_specular = { 0.8, 0.8, 0.8, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\colatex.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);

	++i;
	m.material_shininess = 120.0f;
	m.material_specular = { 0.9, 0.9, 0.9, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\PEPSI.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);

	++i;
	m.material_shininess = 800.0f;
	m.material_specular = { 0.9, 0.9, 0.9, 1.0 };
	_LoadImage("D:\\Документы\\OneDrive\\Документы\\7 семестр\\комп. графика\\MMCS_332\\Individual3_ASR\\textures\\1.jpg");
	render1(Program1, VBO[i], VBN[i], VBT[i], IBO[i], l, m, counts[i], 1);
