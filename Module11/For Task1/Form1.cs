...
namespace Task_3
{
    public partial class Form1 : Form
    {
        ...
        Dictionary<PointPol, int> d = new Dictionary<PointPol, int>();
            ...
            foreach (var pol in fig)
            {
                ...
                if (pol.points.Count() == 3)
                {
                    foreach (var p in pol.points)
                        addPoints(ref points, ref cnt, p);
                ...
                {
                    addPoints(ref points, ref cnt, pol.points[0]);
                    addPoints(ref points, ref cnt, pol.points[1]);
                    addPoints(ref points, ref cnt, pol.points[2]);
                    ...
                    addPoints(ref points, ref cnt, pol.points[0]);
                    addPoints(ref points, ref cnt, pol.points[2]);
                    addPoints(ref points, ref cnt, pol.points[3]);

                    polygons.Add(points);

                }
                else
                    new SystemException("error count");
            }
        }
        ...
        private string save()
        {
            ...
            var d1 = d.Keys.OrderBy(x => d[x]).ToList();

            foreach (var p in d1)
                result += "v " + p.X.ToString() + ' ' + p.Y.ToString() + ' ' + p.Z.ToString() + Environment.NewLine;
